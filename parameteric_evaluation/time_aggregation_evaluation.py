...
class TimeAggregationEvaluator(ParametricEvaluator):
    ...
    @classmethod
    def calculate_self_consumption_for_various_time_aggregations(cls, evaluation_parameters):
        ...
        for n_fam in evaluation_parameters.number_of_families:
            results.loc[n_fam, 'sc_tou'] = PhysicalParameterEvaluator.calculate_theoretical_limit_of_self_consumption(tou_months, n_fam)
            for label, tr in time_resolution.items():
                results.loc[n_fam, label] = PhysicalParameterEvaluator.calculate_sc_for_time_aggregation(energy_year, tr, n_fam)
