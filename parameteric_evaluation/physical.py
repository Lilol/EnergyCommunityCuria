...
from parameteric_evaluation.evaluation import Evaluate, Calculator
...
class CalculatePhysicalParameter(Calculator):
...
class SharedEnergy(CalculatePhysicalParameter):
    _parameter_calculated = Parameter.SHARED_ENERGY

    def calculate(self, input_da: OmnesDataArray, *args):
        return input_da.sel([DataKind.P_INJ, DataKind.P_WITH]).min().sum()


class SelfConsumption(CalculatePhysicalParameter):
    _parameter_calculated = Parameter.SELF_CONSUMPTION

    def calculate(self, input_da: OmnesDataArray, *args):
        return input_da.sel(DataKind.P_SH).sum() / input_da.sel(DataKind.P_INJ).sum()


class SelfSufficiency(CalculatePhysicalParameter):
    _parameter_calculated = Parameter.SELF_SUFFICIENCY

    def calculate(self, input_da: OmnesDataArray, *args):
        return input_da.sel(DataKind.P_SH).sum() / input_da.sel(DataKind.P_WITH).sum()


class EvaluatePhysicalParameters(Evaluate):
    _name = "physical_parameter_evaluation"
    _parameters = CalculatePhysicalParameter.calculators

    def execute(self, dataset: OmnesDataArray, *args, **kwargs) -> OmnesDataArray:
        for parameter, calculator in self._parameters.items():
            calculator.calc(dataset, dataset)
        return dataset
