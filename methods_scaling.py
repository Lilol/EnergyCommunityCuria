...
def scale_seteq(x, nd, y_ref):
    ...
    e_ref = np.concatenate([
        np.array([[y_ref[ij, arera[ij] == f].sum() * nd[ij]
                   for ij in range(nj)]])
        for _, f in enumerate(fs)], axis=0)
...
# This methods evaluates the load profile using a simple quadratic optimization
...
# Moreover other contraints can be added.
def scale_qopt(x, nd, y_ref,
               y_max = None,
               obj = 0,
               obj_reg = None,
               cvxopt = {}):
    ...
    if y_max:
        assert isinstance(y_max, (float, int)) and y_max > 0, \
            "If given, 'y_max' must be a positive number."
    ...
    if obj_reg:
        assert isinstance(obj_reg, float), \
            "If given 'obj_reg' must be a float (weight)."
        l = np.zeros((nh, nh))
        for i_h in range(nh):
            l[i_h, i_h] = 1
            l[i_h, (i_h+1)%nh] = -1
        p += obj_reg * np.dot(l.T, l)
        # q += np.zeros((nh,))
    ...
    h = opt.matrix(h)
    # options for solver
    cvxopt = {**dict(kktsolver = 'ldl',
                      options = dict(kktreg = 1e-9, show_progress = False)),
              **cvxopt}
...
if __name__ == "__main__":
    ...
    # reference profile assigned
    ref = 'office'
    y_ref_db = dict(
        office = np.array(
            [0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.3, 0.4, 0.65, 0.9, 0.95, 1, 1,
             0.95, 0.9, 0.85, 0.65, 0.45, 0.4, 0.35, 0.25, 0.25, 0.25, 0.25]),
        sport_centre = np.array(
            [0.3, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.35, 0.45, 0.55, 
             0.6, 0.6, 0.6, 0.6, 0.6, 0.7, 0.85, 1, 1, 1, 0.95, 0.75, 0.55]),
        )
    ...
    # plot profiles
    ax.plot(time, y_gse, label='Scale, gse',
            marker='s', lw=2, ls='-',)
    ax.plot(time, y_seteq, label='Scale, seteq',
            marker='s', lw=2, ls='-',)
    ax.plot(time, y_qopt, label='Scale, qopt',
            marker='s', lw=2, ls='-',)
