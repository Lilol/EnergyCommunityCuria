...
pie_chart_kw = dict(labels_pos='legend',
                    autopct="absolute",
...
months_labels = ['Gen', 'Feb', 'Mar', 'Apr', 'Mag', 'Giu', 'Lug', 'Ago',
                 'Set', 'Ott', 'Nov', 'Dic']

# Define colors dict to assign to each municipality
colors_municipality = dict(zip(municipalities, get_colors_from_map(
    n_municipalities, cmap_municipality)))
...
colors_tou = dict(F0=cm.get_cmap('nipy_spectral')(0.2),
                  F1=cm.get_cmap('nipy_spectral')(0.85),
                  F2=cm.get_cmap('nipy_spectral')(0.70),
                  F3=cm.get_cmap('nipy_spectral')(0.55))
...
subplots_adjust_kw = dict(left=0, right=1, bottom=0.05, top=0.9,
                          hspace=0)  # (SETUP)
...
for i, data in enumerate(datas):
    col = cols[i]
    counts, labels = zip(*[(count, label) for label, count in data[col]\
...
for i, (counts, labels) in enumerate(zip(pies_counts, pies_labels)):
    ...
    pie_chart(counts, labels=labels, ax=ax, colors=colors_municipality,
              **this_pie_chart_kw)
...
titles = [f'Consumo annuo ({unit_energy})',
          f'Produzione annua ({unit_energy})']  # (SETUP)
subplots_adjust_kw = dict(left=0, right=1, bottom=0.05, top=0.9,
                          hspace=0)  # (SETUP)
...
pies_labels = []
for i, data in enumerate(datas):
    counts, labels = zip(*[(count, label) for label, count in
                           data.groupby(cols_label[i]).sum()[cols_data[i]] \
                         .sort_index().items()])
...
subplots_adjust_kw = dict(left=0.01, right=0.98, bottom=0.05, top=0.9,
                          hspace=0.5)  # (SETUP)
ylabel = "Numero"  # (SETUP)
...
labels_from_bins = lambda bins: [f'<{b}' if j == 0 else
                                 ...
                                 for j, b in enumerate(bins[1:])]  # (SETUP)
...
# Plot data
for i, (counts, labels, legend) in enumerate(zip(bars_counts, bars_labels,
                                                 legend_labels)):
    # Select axis
    ax = axes[i]
    # Plot bars
    ...
    for count, legend in zip(counts, legend):
        ax.bar(range(len(labels)), count, label=legend, bottom=bottom,
               color=colors_municipality[legend])
...
subplots_adjust_kw = dict(left=0.01, right=0.98, bottom=0.02, top=0.9,
                          wspace=0.25, hspace=0.25)  # (SETUP)
...
for i, (counts, labels, legend) in enumerate(zip(bars_counts, bars_labels,
    ...
    # Do not plot zeros
    new_labels = [l for j, l in enumerate(labels) if any([count[j] > 0
                                                          for count in counts])]
    ...
    bottom = [0] * len(labels)
    for count, legend in zip(counts, legend):
        ax.bar(labels, count, label=legend, bottom=bottom,
               color=colors_types[legend])
        bottom = [b + c for b, c in zip(bottom, count)]
...
for i, (counts, labels, legend) in enumerate(zip(bars_counts, bars_labels,
    ...
    # Do not plot zeros
    new_labels = [l for j, l in enumerate(legend) if any([count[j] > 0
                                                          for count in counts])]
...
subplots_adjust_kw = dict(left=0.1, top=0.95, right=0.95, bottom=0.15,
...
# Plot data
for j, (counts, labels, legends) in enumerate(zip(bars_counts, bars_labels,
                                                legend_labels)):
    k = 0
    for i, (count, legend) in enumerate(zip(counts, legends)):
        ...
        # Plot bars
        ax.bar(labels, count, label=legend, align=align, width=width,
               color=colors_tou[legend])
...
lw = 2
path_effect = lambda lw: [pe.Stroke(linewidth=1.5*lw, foreground='w'),
                          pe.Normal()]
c_users = 'tab:blue'
c_plants = 'tab:red'
c_shared = 'tab:green'
...
subplots_adjust_kw = dict(top=0.92, right=0.98, bottom=0.15, left=0.12,
...
subplots_adjust_kw = dict(top=0.92, right=0.98, bottom=0.12, left=0.12,
...
for i in range(len(titles)):
    if i == 0:
        ...
        axes.append(fig.add_subplot(spec[i//ncols, i%ncols],
                                    sharex=axes[-1], sharey=axes[-1]))
ax_leg = fig.add_subplot(spec[:, -1])
...
for i, season in enumerate(profiles_plants):
    ...
    # Plot profiles
    ax.plot(t_plot, p_plants, label='$P_\mathrm{impianti}$',
            color=c_plants, lw=lw,
            path_effects=path_effect(lw))
    ax.plot(t_plot, p_users, label='$P_\mathrm{utenze}$',
            color=c_users, lw=lw,
            path_effects=path_effect(lw))
    ax.fill_between(t_plot, p_plants, p_users, where=p_plants < p_users,
                    label='$E_\mathrm{\\leftarrow rete}$',
                    color=c_fromgrid, alpha=alpha)
    ax.fill_between(t_plot, p_plants, p_users, where=p_plants > p_users,
                    label='$E_\mathrm{\\rightarrow rete}$',
                    color=c_togrid, alpha=alpha)
    ax.fill_between(t_plot, np.minimum(p_plants, p_users),
                    label='$E_\mathrm{condivisa}$',
                    color=c_shared, alpha=alpha)
...
s_min, s_max = 100, 500
scatter_kw = dict(marker='s', s=[s_min + (s_max-s_min) * (x_ - min(x)) / \
                                 (max(x)-min(x)) for x_ in x], zorder=2)
...
for i, (hline, vline) in enumerate(zip(hlines, vlines)):
    if i > 0:
        ...
        ax.annotate("", (vline, y_min), (vline, y_min+0.01*y_min),
                    arrowprops=arrow_kw)
...
subplots_adjust_kw = dict(top=0.9, right=0.98, left=0.1, bottom=0.23,
                          wspace=0.2)
...
tickparams_kw = dict(axis='x', rotation=60)
xlabel_kw = dict(x=0.5, y=0.035, s=xlabel, ha='center', va='center')
subplots_adjust_kw = dict(top=0.98, right=0.98, left=0.11, bottom=0.23,
                          wspace=0.37)
